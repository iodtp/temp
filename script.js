...
function training(tiles, spawn, map, value){
    ...
    const player2 = addPlayer(app, tiles, [60,80], world);
    ...
    if(spikeMaze){
        ...
        goalGraphics.beginFill(0xff0000); // Red color
        goalGraphics.drawCircle(0, 0, 10); // Radius 10 pixels
        goalGraphics.endFill();
        goalGraphics.x = goalX * 40;
        goalGraphics.y = goalY * 40;
...
async function AILoop(delta, players, world, keys, app, model, agents, goalX, goalY) {
    ...
    if (frameCount >= FRAMES_PER_EPISODE) {
        gen++;
        evaluateAndTrain(agents, gen);

        frameCount = 0;
        resetEnvironment(players);
    }
...
function evaluateAndTrain(agents, iter) {
    agents.sort((a, b) => b.reward - a.reward);
    console.log("Best reward generation", iter, ": ", agents[0].reward);
...
function zoomToFitMap(app, mapWidth, mapHeight) {
    const scaleX = app.renderer.width / mapWidth;
    const scaleY = app.renderer.height / mapHeight;
    ...
    app.stage.position.set(
        (app.renderer.width - mapWidth * scale) / 2,
        (app.renderer.height - mapHeight * scale) / 2
    );
}
